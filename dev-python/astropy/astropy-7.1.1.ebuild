...
python_prepare_all() {
	...
	if use doc && ! use intersphinx; then
		...
		for ddv in "${DISTDIR}"/*-dv-*; do { cp ${ddv} "${S}"/docs/whatsnew/${ddv##*-dv-} || die ; } ; done
...
pkg_postinst() {
	...
	optfeature "Used in astropy.table.Table.show_in_notebook() to display the Astropy table in Jupyter notebook for backend=\"ipydatagrid\"" dev-python/ipydatagrid
...
default indexing engine" dev-python/sortedcontainers
	...
	optfeature "provide plotting functionality that astropy.visualization enhances" ">=dev-python/matplotlib-3.9.1"
