...
PATCHES=(
	"${FILESDIR}"/${MY_PN}-2.4.17-gcc44.patch

	"${FILESDIR}"/${MY_PN}-2.2.14-perlthreadsfix.patch
	"${FILESDIR}"/${MY_PN}-2.4.15-ppolicy.patch

	# bug #116045 - still present in 2.4.28
	"${FILESDIR}"/${MY_PN}-2.4.35-contrib-smbk5pwd.patch
	# bug #408077 - samba4
	"${FILESDIR}"/${MY_PN}-2.4.35-contrib-samba4.patch

	# bug #189817
	"${FILESDIR}"/${MY_PN}-2.4.11-libldap_r.patch

	# bug #233633
	"${FILESDIR}"/${MY_PN}-2.4.45-fix-lmpasswd-gnutls-symbols.patch

	# bug #281495
	"${FILESDIR}"/${MY_PN}-2.4.28-gnutls-gcrypt.patch

	# bug #294350
	"${FILESDIR}"/${MY_PN}-2.4.6-evolution-ntlm.patch

	# unbreak /bin/sh -> dash
	"${FILESDIR}"/${MY_PN}-2.4.28-fix-dash.patch

	# bug #420959
	"${FILESDIR}"/${MY_PN}-2.4.31-gcc47.patch

	# unbundle lmdb
	"${FILESDIR}"/${MY_PN}-2.4.42-mdb-unbundle.patch
...
multilib_src_install_all() {
	rm -r "${ED}"/usr/bin || die
	use minimal || { rm -r "${ED}"/usr/sbin || die ; }
	rm -r "${ED}"/usr/include || die
	rm -r "${ED}"/etc || die
	use minimal || { rm -r "${ED}"/usr/lib*/openldap || die ; }
