...
#
# To create a vendor tarball, simply use the script here: https://github.com/bekcpear/vendor-for-go
# The vendor directory in ${EGO_VENDOR_DIR} will be moved to the ${S} directory.
# Note the name of go-mod-sum.diff in ${EGO_VENDOR_DIR} should be kept if it exists.
#
...
# Add this line if you have a vendor tarball.
...
# @FUNCTION: go-module-vendor_set_globals
# @DEPRECATED: none
# @DESCRIPTION:
...
go-module-vendor_set_globals() {
	...
	for line in "${EGO_SUM[@]}"; do
		...
		_dir=$(_go-module-vendor_gomod_encode "${module}")
...
# @FUNCTION: go-module-vendor_go_version
# @USAGE: [-f]
# @DESCRIPTION:
...
# @FUNCTION: go-module-vendor_setup_proxy
# @DEPRECATED: none
# @DESCRIPTION:
...
go-module-vendor_setup_proxy() {
	...
	for f in ${A}; do
		...
		if [[ -n "${goproxy_mod_path}" ]]; then
			...
			v="${v%.zip}"
			v="${v//*\/}"
			_go-module-vendor_gosum_synthesize_files "${goproxy_mod_dir}" "${v}"
		fi
	done
...
# @FUNCTION: go-module-vendor_setup_vendor
# @DESCRIPTION:
...
go-module-vendor_setup_vendor() {
	...
	if [[ ! -d "${S}/vendor" ]]; then
		if [[ ${PROPERTIES} =~ (^|[[:space:]])live([[:space:]]|$) ]]; then
			...
			edo go mod tidy -compat $(go-module-vendor_go_version)
			edo go mod vendor
			popd >/dev/null || die
		else
			local -a vendors
...
# - .../@v/${version}.info
# - .../@v/list
_go-module-vendor_gosum_synthesize_files() {
	local target=$1
	local version=$2
...
# @FUNCTION: go-module-vendor_live_vendor
# @DESCRIPTION:
...
# @FUNCTION: _go-module-vendor_gomod_encode
# @DEPRECATED: none
# @DESCRIPTION:
...
#
_go-module-vendor_gomod_encode() {
	## Python:
